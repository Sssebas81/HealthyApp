...
export default function ChatBot({ objetivo, preferencias }: ChatBotProps) {
  ...
  const generarPlanAlimenticio = () => {
    setTimeout(() => {
      ...
      if (objetivo === "perder_peso") {
        ...
        plan = {
          desayuno:
            "3 claras de huevo revueltas + 1 rebanada de pan integral + 1/2 aguacate (350 kcal)",
          almuerzo:
            "150g pechuga de pollo a la plancha + 100g quinoa + ensalada verde (500 kcal)",
        ...
        plan = {
          desayuno:
            "4 claras + 2 huevos enteros + 50g avena + 1 banana (600 kcal)",
          almuerzo:
            "200g carne magra + 200g arroz integral + 100g brócoli (700 kcal)",
      ...
      const preferenciasActivas =
        Object.keys(preferencias)
          .filter((k) => preferencias[k as keyof Preferencias])
          .join(", ") || "Ninguna específica";
  ...
  const enviarMensaje = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setMensajes((prev) => [
      ...prev,
      { tipo: "usuario", contenido: mensaje, timestamp: new Date() },
    ]);
    input.value = "";
    ...
    try {
      const response = await fetch("/api/chat-ia", {
        ...
        body: JSON.stringify({
          ...
          contexto: {
            objetivo,
            preferencias: Object.keys(preferencias).filter(
              (k) => preferencias[k as keyof Preferencias],
            ),
          },
        }),
      });
      ...
      setMensajes((prev) => [
        ...
        {
          tipo: "ia",
          contenido: "Lo siento, hubo un error de conexión.",
          timestamp: new Date(),
        },
      ]);
    }
